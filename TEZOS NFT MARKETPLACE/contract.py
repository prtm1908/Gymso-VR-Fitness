...
# Import the contents of 'fa2_lib.py' directly
from fa2_lib import FA2
...
class Marketplace(sp.Contract):
    ...
    def mint(self, params):
        ...
        args = [sp.record(to_=sp.self_address, metadata={'': params.metadata})]
...
def test():
    ...
    # Use the FA2 contract directly, no need for 'import_template'
    token_contract = FA2(admin=admin.address, metadata=sp.utils.metadata_of_url("ipfs://QmW8jPMdBmFvsSEoLWPPhaozN6jGQFxxkwuMLtVFqEy6Fb"))
    ...
    marketplace.mint(
        amount=sp.tez(100),
        metadata=sp.pack("ipfs://bafyreibwl5hhjgrat5l7cmjlv6ppwghm6ijygpz2xor2r6incfcxnl7y3e/metadata.json")).run(sender=admin, valid=False, exception="FA2_NOT_ADMIN")

    token_contract.set_administrator(marketplace.address).run(sender=admin)

    marketplace.mint(
        amount=sp.tez(100),
        metadata=sp.pack("ipfs://bafyreibwl5hhjgrat5l7cmjlv6ppwghm6ijygpz2xor2r6incfcxnl7y3e/metadata.json")).run(sender=admin)
